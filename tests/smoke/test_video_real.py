...
def test_real_seedance_generates_one_four_second_low_resolution_cut() -> None:
    if os.getenv("RUN_REAL_VIDEO_SMOKE") != "1":
        pytest.skip("set RUN_REAL_VIDEO_SMOKE=1 to create one paid Seedance task")
    settings = Settings()
    if not settings.video_api_key or not settings.video_base_url or not settings.video_model:
        pytest.skip("Video Provider is not configured")
    ...
    request_file = smoke_dir / "seedance_request_id.txt"
    ...
    output = smoke_dir / "seedance_smoke.mp4"
    output.write_bytes(video)
    metadata = FFmpegRenderProvider().probe(output)
    (smoke_dir / "seedance_result.json").write_text(
        json.dumps(
            {
    ...
    assert metadata.duration_ms == pytest.approx(4_000, abs=1_000)
